  ...
  return (
    ...
    >
      <article className="relative aspect-[2/3] overflow-hidden rounded-sm border border-gold/20 group-hover:border-gold/80 transition-all duration-500">
        ...
        <span className="absolute inset-0 border border-gold/15 m-2 pointer-events-none z-10 hidden sm:block" />

        {/* Film strip pattern on edges */}
        <div className="absolute top-0 bottom-0 left-0 w-2 bg-bg-deep hidden sm:flex flex-col justify-around py-2 z-10">
          {Array.from({ length: 14 }).map((_, i) => (
            <span key={i} className="block w-1 h-1.5 mx-auto bg-gold/40 rounded-full" />
          ))}
        </div>
        <div className="absolute top-0 bottom-0 right-0 w-2 bg-bg-deep hidden sm:flex flex-col justify-around py-2 z-10">
          {Array.from({ length: 14 }).map((_, i) => (
            <span key={i} className="block w-1 h-1.5 mx-auto bg-gold/40 rounded-full" />
          ))}
        </div>
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-3 sm:p-5 md:p-6">
          ...
          <div className="absolute inset-x-0 bottom-0 p-3 sm:p-5 md:p-6 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500 bg-gradient-to-t from-bg-deep via-bg-deep/95 to-bg-deep/30">
